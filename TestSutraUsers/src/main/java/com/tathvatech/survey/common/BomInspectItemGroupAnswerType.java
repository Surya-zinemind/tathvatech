...
public class BomInspectItemGroupAnswerType extends SurveyItem implements BaseInspectionItemGroupAnswerType, SurveyDisplayItem,
		LogicSubject
{
	private static final Logger logger					= LoggerFactory.getLogger(BomInspectItemGroupAnswerType.class);
	...
	public static final String	HEADING_ACTUAL			= "Actual";
	...
	public static String FLAG_SHOWALL = "OPT_SHOW_ALL_ITEMS";
	public static String FLAG_SHOWFAILONLY = "OPT_SHOW_FAILED_ONLY";
	public static String FLAG_SHOWTRANSFERREDITEMSONLY = "OPT_SHOW_TRANSFERRED_ONLY";
	public static String FLAG_SHOWCOMMENTONLY = "OPT_SHOW_COMMENTS_ONLY";
	public static String FLAG_SHOWUNANSWEREDONLY = "OPT_SHOW_UNANSWERD_ONLY";
	public static String FLAG_SHOWUNANSWEREDBUTMANDATORYONLY = "OPT_SHOW_UNANSWERD_BUT_MANDATORY_ONLY";
	public static String FLAG_SHOW_FLAGGED_ITEMS_ONLY = "OPT_SHOW_FLAGGED_ITEMS__ONLY";
	public static String FLAG_SHOWGRAPHBUTTON = "OPT_SHOW_GRAPH_BUTTON";
	public static String FLAG_SHOWPREVIOUS_ENTRIES_BUTTON = "OPT_SHOW_PREVIOUS_ENTRIES_BUTTON";
	public static String FLAG_EDIT_COMMENTS_ONLY = "EDIT_COMMENT_FIELD";
	...
	boolean showActualField = false;
	boolean showUnit = false;
	boolean showPassFail = false;
	boolean showComments = false;
	private final BomInspectItemAnswerType aChild;
     ...
     *
     */
	public BomInspectItemGroupAnswerType(BomInspectItemAnswerType aChild)
	{
		super();
	 ...
	 * @param _survey
	 */
	public BomInspectItemGroupAnswerType(SurveyDefinition _survey, BomInspectItemAnswerType aChild)
	{
		super(_survey);
        this.aChild = aChild;
    }
	...
	{
		return "BomInspectItemGroupAnswerType";
	}
	...
	public boolean isShowActualField()
	{
		return showActualField;
	}

	public void setShowActualField(boolean showActualField)
	{
		this.showActualField = showActualField;
	}
	...
	public void setConfiguration(Element element)
	{
		...
		this.setShowActualField(new Boolean(element.getAttributeValue("showActualField")).booleanValue());
...
//						fElement.getAttributeValue("textDesc"));
	...
	private void setConfigurationInt(List<List<String>> fileContents)
	{
		...
		for (int i = 0; i < heading.size(); i++)
		{
			...
			else if (HEADING_ACTUAL.equalsIgnoreCase(aHeading))
			{
				actualColIndex = i;
			}
...
//			}
			...
			try
			{
				if("y".equalsIgnoreCase(row.get(actualColIndex).trim()))
				{
					aChild.setShowActualValue(true);
					showActualField = true;
				}
			}
			catch (Exception e)
			{
			...
			{
				if(row.size() > customFields.getOptionByIndex(k).getValue())
				{
					Option aOption = new Option(customFields.getOptionByIndex(k).getText(), row.get(customFields.getOptionByIndex(k).getValue()));
					childCustomFields.add(aOption);
				}
				else
				{
					Option aOption = new Option(customFields.getOptionByIndex(k).getText(), "");
					childCustomFields.add(aOption);
				}
			}
		...
		// now remove the cols that children dont have from the optionList
		if(showExpectedField == false)
		{
			customFields.removeByName(HEADING_EXPECTED);
		}
		if(showActualField == false)
		{
			customFields.removeByName(HEADING_ACTUAL);
		}
		if(showUnit == false)
		{
			customFields.removeByName(HEADING_UNIT);
		}
		if(showPassFail == false)
		{
			customFields.removeByName(HEADING_RESULT);
		}
		if(showComments == false)
		{
			customFields.removeByName(HEADING_COMMENTS);
		}
		//now iterate through the children and remove the unused custom fields from them too
		for (Iterator iterator = children.iterator(); iterator.hasNext();)
		{
			BomInspectItemAnswerType aItem = (BomInspectItemAnswerType) iterator.next();
			OptionList cFields = aItem.getCustomFields();
			if(showExpectedField == false)
			{
				aItem.setShowExpectedValue(false);
				cFields.removeByName(HEADING_EXPECTED);
			}
			if(showActualField == false)
			{
				aItem.setShowActualValue(false);
				cFields.removeByName(HEADING_ACTUAL);
			}
			if(showUnit == false)
			{
				aItem.setShowUnit(false);
				cFields.removeByName(HEADING_UNIT);
			}
			if(showPassFail == false)
			{
				aItem.setShowPassFail(false);
				cFields.removeByName(HEADING_RESULT);
			}
			if(showComments == false)
			{
				aItem.setShowComments(false);
				cFields.removeByName(HEADING_COMMENTS);
			}
		}
	}
	/*
	...
	public Element toXML()
	{
		...
		element.setAttribute("showActualField", new Boolean(showActualField).toString());
...
//commenting methods using vaadin components
			...
			boolean expandedView, boolean isLatestResponse, String[] flags, final TestProcController testProcController)
	{
		...
		for (Iterator iterator = customFields.iterator(); iterator.hasNext();)
		{
			...
			else if (HEADING_ACTUAL.equalsIgnoreCase(headingText))
			{
				if(showActualField)
				{
...
//		table.setVisibleColumns(vCols.toArray(new String[vCols.size()]));

		int childTableIndex = 0;
		for (int i = 0; i < children.size(); i++)
		{
			BomInspectItemAnswerType aChild = (BomInspectItemAnswerType) children.get(i);
			try
			{
...
//		table.setPageLength(pageLength);
		for (Iterator iterator = customFields.iterator(); iterator.hasNext();)
		{
			...
			else if (HEADING_ACTUAL.equalsIgnoreCase(headingText))
			{
				if(showActualField)
				{
					table.addContainerProperty(aField.getText(), TextField.class, null);
					table.setColumnExpandRatio(aField.getText(), colExpandRatio);
				}
			}
			else if (HEADING_UNIT.equalsIgnoreCase(headingText))
		...
		for (int i = 0; i < children.size(); i++)
		{
			final BomInspectItemAnswerType aChild = (BomInspectItemAnswerType) children.get(i);
...
/*
			...
			Upload.FailedListener
	{
		...
		BomInspectItemGroupAnswerType	surveyItem;

		BeanFieldGroup<BomInspectItemGroupAnswerType> fieldGroup;
		...
		public ConfigForm(BomInspectItemGroupAnswerType surveyItem,
				FormDesignListener formDesignListener)
		{
			...
			BeanItem<BomInspectItemGroupAnswerType> item = new BeanItem<BomInspectItemGroupAnswerType>(
					surveyItem);

			fieldGroup = new BeanFieldGroup<BomInspectItemGroupAnswerType>(BomInspectItemGroupAnswerType.class);
...
//				this.setComponentError(new UserError(iv.getMessage()));
		...
		{
			// does not contain the question text and desc
		...
		private void showItemPreview()
		{
			...
			filePreviewArea.addComponent(l1);
...
//
//			Window w = ((Window)filePreviewArea.findAncestor(Window.class));
//			w.setWidth("80%");
//			w.setPositionX(10);
		}
			...
			FormEventListner formEventListner)
	{
		...
		table.setColumnAlignment(HEADING_ATTACH, Table.ALIGN_CENTER);
